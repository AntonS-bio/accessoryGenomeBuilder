...
def runBlast(graphNode):
    ...
    for gene in allNodes[graphNode]:
        ...
        for sequence in geneSequences.stdout.decode().split("\t"):
            blastNucleotide=subprocess.run("blastn -query <(printf \'"+sequence+"\' ) -task 'megablast' -db IS -max_target_seqs 1000000000 -num_threads 4 -evalue 0.0000000001 -word_size 11 -outfmt \" 6 delim=  qseqid qstart qend sseqid evalue pident \" ", executable="/bin/bash", shell=True, stdout=subprocess.PIPE)
...
def run(AMRDir, AMRfile, outputDir, PassedSamplesDir):
    ...
    counter=0
    subprocess.call("pkill -9 identifyISgenes", shell=True) #cleans up existing running threads if any.
    ...
    subprocess.call("makeblastdb -in "+ AMRDir+"IS.fasta -blastdb_version 4 -title IS -out IS -dbtype nucl", shell=True)


    if __name__ == 'identifyISgenes':
        global allNodes
        ...
        print("pickling")
        pickle.dump( joinedWithinResults, open(outputDir+"joinedWithinResultsIS.pkl", "wb") )
        pickle.dump( joinedBetweenResults, open(outputDir+"joinedBetweenResultsIS.pkl", "wb") )

        joinedWithinResults=pickle.load( open(outputDir+ "joinedWithinResultsIS.pkl", "rb" ) )
        joinedBetweenResults=pickle.load( open(outputDir+ "joinedBetweenResultsIS.pkl", "rb" ) )
        ...
        newMetaData=open(outputDir+"MetaDataIS.tsv","w")
        allResults={**joinedWithinResults, **joinedBetweenResults}
        allCliques= {**betweenNodes, **withinNodes}
        print(refGenesMetaData)
        for line in open(outputDir+"MetaData.tsv"):
